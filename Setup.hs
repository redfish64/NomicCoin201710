{-| most of this code is copied directly from idris's Setup.hs
    We pick and choose what we want to actually do for AN.
 -}
...
main =
  do
    putStrLn "HI THEEREREREEREREERE!!!!!!!"
    defaultMainWithHooks $ simpleUserHooks
      {
        postConf = anConfigure
      }
...
generateVersionModule verbosity dir release = do
    hash <- gitHash
    let versionModulePath = dir </> "Version_AN201710" Px.<.> "hs"
  ...
  where versionModuleContents h = "module Version_AN201710 where\n\n" ++
                                  "gitHash :: String\n" ++
                                  if release
                                    then "gitHash = \"\"\n"
                                    else "gitHash = \"git:" ++ h ++ "\"\n"
...
anConfigure _ flags pkgdesc local = do
    putStrLn "anCONFIGURE!!!!!!!"
    --configureRTS   -- TODO 3 we don't need a runtime system until we enable compilation
    let libAutogenDir = autogenModulesDir local
    generateVersionModule verbosity libAutogenDir (isRelease (configFlags local))
  where
      verbosity = S.fromFlag $ S.configVerbosity flags
      version   = pkgVersion . package $ localPkgDescr local
